...
import { LightBulbIcon, TrendUpIcon, TrendDownIcon } from './icons';
...
const PickCard: React.FC<{ pick: TodaysPick; type: 'Buy' | 'Sell'; onAnalyze: (symbol: string) => void }> = ({ pick, type, onAnalyze }) => {
...
const TodayRecommendation: React.FC<TodayRecommendationProps> = ({ onFetchPicks, picks, loading, error, onAnalyze, isAppLoading }) => {
  return (
      ...
      <div className="p-6 bg-gray-800/50 border border-gray-700 rounded-xl">
        ...
        {picks && !loading && (
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 text-left animate-fade-in">
                <PickCard pick={picks.buy} type="Buy" onAnalyze={onAnalyze} />
                <PickCard pick={picks.sell} type="Sell" onAnalyze={onAnalyze} />
            </div>
        )}
      </div>
    </section>
