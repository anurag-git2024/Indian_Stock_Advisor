...
export const fetchStockAnalysis = async (stockSymbol: string): Promise<StockAnalysis> => {
    const prompt = `
    ...
    try {
        ...
        if (!parsedData.stock_name || !parsedData.current_price || !Array.isArray(parsedData.analysis) || !Array.isArray(parsedData.top_news)) {
